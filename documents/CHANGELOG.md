...
- **Contrast.** Every one of 33 role pairs passes WCAG 2.2 AA in both themes (66 checks), with the ratios published in `guidelines.md`. `prefers-contrast: more` is supported.
...
- **Accessibility.** Guidelines now cover focus (2px at 2px offset), 44px targets, RTL mirroring rules, and a WCAG 2.2 AA checklist. A solid callout ground or an inverse block carries `data-ds-surface`, which re-points the one focus ring to a step that still holds 3:1 on it.
