...
- **Contrast.** Every one of 31 role pairs passes WCAG 2.2 AA in both themes (62 checks), with the ratios published in `guidelines.md`. `prefers-contrast: more` is supported.
...
- **Accessibility.** Guidelines now cover focus (2px at 2px offset), 44px targets, RTL mirroring rules, and a WCAG 2.2 AA checklist.
- **Preview.** Added `preview.html`, a self-contained page showing the tokens, both themes side by side, depth, motion and the key components. It uses system font fallbacks and makes no external requests.
- **Compatibility.** Every 1.x variable is kept with its 1.x value, marked deprecated, with a migration section in `guidelines.md`. The screen caption floor rises from 12px to 14px.
