...
- **Contrast table.** Every text, control, focus and chart pair in both themes, with its ratio, in `guidelines.md`. Target moves from WCAG 2.1 AA to 2.2 AA. The focus ring moves one step darker so it passes 3:1 on light wells, and takes `--ds-focus-inverse` on the inverse surface so a tooltip and the bulk-action bar keep a legible ring. `prefers-contrast: more` lifts the decorative line roles and muted text in both themes, and those pairs are measured too.
