...
- **Colour roles.** Components now read roles (surface, ink, line, user, ink action, machine, status, focus, data) instead of palette steps. The warm accent is reserved for the user (their one primary action, their choices, where they are, their own chart item) and no longer carries links, card borders, card hover glows or chart series. Secondary actions and links are ink.
